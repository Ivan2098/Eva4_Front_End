...
const UsersAdd = () => {
    ...
    const cancel = () => {
        var {operatorId, userName, nickname, password, level} = document.forms[0]; 
        var hasChanges = operatorId.value.length > 0 ||  userName.value.length > 0 || nickname.value.length > 0 || password.value.length > 0 ||
    ...
    const save = async (event) => {
        ...
        var errors = "";
        errors += userName.value.length === 0 ? "El campo 'Usuario' es obligatorio.\n" : "";
        ...
        if(errors.length > 0){
            ...
            const requestOptions = {
                ...
                body: JSON.stringify({ "operatorId": config.operatorId,"userName": userName.value,"nickname": nickname.value,"password": password.value,"level": level.value})
              }
