...
const UsersEdit = () => {
    ...
    const cancel = () => {
        var {usersName, MSU, price, stock, MDPrice, MDPercentage} = document.forms[0]; 
        var hasChanges = usersName.value.length > 0 ||  MSU.value.length > 0 || price.value.length > 0 || stock.value.length > 0 ||
    ...
    const save = async (event) => {
        ...
        if(errors.length > 0){
            ...
            const requestOptions = {
                ...
                body: JSON.stringify({ "operatorId": config.operatorId, "name": usersName.value,"MSU": MSU.value,"price": price.value,"stock": stock.value,
                "MDPrice": MDPrice.value,"MDPercentage": MDPercentage.value, "active": usersData.active})
              }
              fetch(config.apiURL+"userss/"+usersData.id, requestOptions).then((response) => {
                    ...
                    <div className="container-fluid">
                        <div className="row mb-2">
                            <div className="col-sm-6">
                                <h1>Incorporación de userso</h1>
                            </div>
                            <div className="col-sm-6">
                                <ol className="breadcrumb float-sm-right">
                                    <li className="breadcrumb-item"><a href="/">Cloud Sales</a></li>
                                    <li className="breadcrumb-item"><a href="/userss">usersos</a></li>
                            ...
                            <div className="row">
                                <div className="col-12">
                                    <div className="form-group">
                                        ...
                                        <input type="text" name="usersName" id="usersName" className="form-control" defaultValue={usersData.name} required />
                                    </div>
                                </div>
                                <div className="col-4">
                                    <div className="form-group">
                                        <label htmlFor="MSU" className="control-label">Un. Mini. de Venta</label>
                                        <input type="number" name="MSU" id="MSU" className="form-control" defaultValue={usersData.MSU} required />
                                    </div>
                                </div>
                                <div className="col-4">
                                    <div className="form-group">
                                        <label htmlFor="price" className="control-label">Precio</label>
                                        <input type="number" name="price" id="price" className="form-control" defaultValue={usersData.price} required />
                                    </div>
                                </div>
                                <div className="col-4">
                                    <div className="form-group">
                                        <label htmlFor="stock" className="control-label">Stock</label>
                                        <input type="number" name="stock" id="stock" className="form-control" defaultValue={usersData.stock} required />
                                    </div>
                                </div>
                                <div className="col-6">
                                    <div className="form-group">
                                        <label htmlFor="MDPrice" className="control-label">Maximo Descuento Precio</label>
                                        <input type="number" name="MDPrice" id="MDPrice" className="form-control" defaultValue={usersData.MDPrice} required />
                                    </div>
                                </div>
                                <div className="col-6">
                                    <div className="form-group">
                                        <label htmlFor="MDPercentage" className="control-label">Maximo Descuento Porcentaje</label>
                                        <input type="number" name="MDPercentage" id="MDPercentage" defaultValue={usersData.MDPercentage} className="form-control" max="50" min="0" required />
                                    </div>
                                </div>
                            </div>
